...
void printPolynomial(struct Term* head) {
    ...
    while (current != NULL) {
        ...
        else if (current->coeff < 0) printf(" - ");

        int absCoeff = current->coeff < 0 ? -current->coeff : current->coeff;

        if (current->exp == 0) printf("%d", absCoeff);
        else if (current->exp == 1) printf("%dx", absCoeff);
        else printf("%dx^%d", absCoeff, current->exp);
...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (head == NULL) { head = tail = newTerm; }
