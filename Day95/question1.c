...
void bucketSort(float arr[], int n) {
    ...
    for (int i = 0; i < n; i++) buckets[i] = (float*)malloc(n * sizeof(float));

    for (int i = 0; i < n; i++) {
        int idx = (int)(n * arr[i]);
        buckets[idx][sizes[idx]++] = arr[i];
    }
    for (int i = 0; i < n; i++) insertionSortBucket(buckets[i], sizes[i]);

    int index = 0;
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < sizes[i]; j++) arr[index++] = buckets[i][j];
        free(buckets[i]);
    }
